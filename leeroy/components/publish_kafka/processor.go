...
func publishKafkaProcessorConfig() *service.ConfigSpec {
	return service.NewConfigSpec().
		Beta().
		Categories("Services").
		Version("3.61.0").
		Summary("A Kafka processor for publishing messages using the [Franz Kafka client library](https://github.com/twmb/franz-go).").
...
func init() {
	err := service.RegisterProcessor("publish_kafka", publishKafkaProcessorConfig(),
		func(conf *service.ParsedConfig, mgr *service.Resources) (service.Processor, error) {
			return newFranzKafkaWriterFromConfig(conf, mgr.Logger())
		})
	if err != nil {
		panic(err)
	}
...
func (f *franzKafkaWriter) Process(ctx context.Context, message *service.Message) (service.MessageBatch, error) {
	// -- connect if needed
	...
	if topic, err = f.topic.TryString(message); err != nil {
		return nil, fmt.Errorf("topic interpolation error: %w", err)
	}

	record := &kgo.Record{Topic: topic}
	if record.Value, err = message.AsBytes(); err != nil {
		return nil, err
	}
	if f.key != nil {
		if record.Key, err = f.key.TryBytes(message); err != nil {
			return nil, fmt.Errorf("key interpolation error: %w", err)
		}
	}
	if f.partition != nil {
		partStr, err := f.partition.TryString(message)
		if err != nil {
	...
	_ = f.metaFilter.Walk(message, func(key, value string) error {
	...
	pr := f.client.ProduceSync(ctx, record)

	if pr.FirstErr() != nil {
		return nil, fmt.Errorf("failed to send message: %w", pr.FirstErr())
	}
