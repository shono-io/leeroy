...
.PHONY: all deps docker docker-cgo clean fmt install
...
install: $(APPS)
	...
	@rm -f $(INSTALL_DIR)/leeroy
	@cp $(PATHINSTBIN)/* $(INSTALL_DIR)/

deps:
	@go mod tidy

SOURCE_FILES = $(shell find leeroy -type f)
TEMPLATE_FILES = $(shell find templates -type f -name "*.template.yaml")

$(PATHINSTBIN)/%: $(SOURCE_FILES)
	@go build $(GO_FLAGS) -tags "$(TAGS)" -ldflags "$(LD_FLAGS) $(VER_FLAGS)" -o $@ ./$*
...
fmt:
	...
	@go list -f {{.Dir}} ./... | xargs -I{} goimports -w -local github.com/shono-io/leeroy {}
	@go mod tidy
